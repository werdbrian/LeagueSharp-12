...
namespace Teemo___The_Satan_Yordle
{
    class Program
    {
     ...
     //   private static readonly Render.Text _textF = new Render.Text("", 0, 0, 24, SharpDX.Color.Red);
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Menu.SubMenu("LaneClear").AddItem(new MenuItem("LaneClearA", "LaneClear").SetValue(new KeyBind("X".ToCharArray()[0], KeyBindType.Press)));
            ...
            /*
            {
                ...
                Emotes.AddItem(new MenuItem("EmoteToggable", "EmoteSpam (Toggle)")).SetValue(new KeyBind("Y".ToCharArray()[0], KeyBindType.Toggle, false));
                Emotes.AddItem(new MenuItem("EmoteD", "Spam Delay (ms)")).SetValue(new Slider(100, 1000, 1));
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (Menu.Item("ComboActive").GetValue<KeyBind>().Active)
            {
                Combo();
                     ...
                     Menu.Item("HarassActiveT").GetValue<bool>())
            {
                Harass();
            }
...

            {
                ...
                if (Menu.Item("EmoteToggable").GetValue<KeyBind>().Active)
                {
                    SPAM();
                }
            }
      ...
      * Made by : TheKushStyle
       ...
       */
        private static void SPAM()
        {
            ...
            var Delay1 = Menu.Item("EmoteD").GetValue<Slider>().Value;

            if (TypeC == 0)
            {
                Utility.DelayAction.Add(Delay1, () => Packet.C2S.Emote.Encoded(new Packet.C2S.Emote.Struct(2)).Send());

                // Packet.C2S.Move.Encoded(new Packet.C2S.Move.Struct(Game.CursorPos.X, Game.CursorPos.Y)).Send(); // dont need it
            }
            else if (TypeC == 1)
            {
                Utility.DelayAction.Add(Delay1, () => Packet.C2S.Emote.Encoded(new Packet.C2S.Emote.Struct(1)).Send());

            }
            else if (TypeC == 2)
            {
                Utility.DelayAction.Add(Delay1, () => Packet.C2S.Emote.Encoded(new Packet.C2S.Emote.Struct(3)).Send());
   ...
   */
        private static void Orbwalking_AfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            ...
            if (Menu.Item("LaneClearA").GetValue<KeyBind>().Active)
            {
    ...
    */
        private static void Combo()
        {
            var TG = TargetSelector.GetTarget(Q.Range, TargetSelector.DamageType.Physical);
            var PacketE = Menu.Item("Packets").GetValue<bool>();
            var TGW = TargetSelector.GetTarget(Orbwalking.GetRealAutoAttackRange(ObjectManager.Player), TargetSelector.DamageType.Physical);
